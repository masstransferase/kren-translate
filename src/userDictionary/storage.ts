...
export interface UserDictionaryStorageOptions {
  ...
  // A seam for the lock handle's close, matching commitTemporaryFile. It exists so a
  // failing close can be exercised: releasing the lock must survive one.
  closeLockHandle?: (handle: { close: () => Promise<void> }) => Promise<void>;
}
...
export class UserDictionaryStorage {
  ...
  private readonly closeLockHandle: (handle: { close: () => Promise<void> }) => Promise<void>;
  ...
  private async withLock<T>(operation: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      // close() is nested in its own try/finally so that a failure to close the handle
      // cannot skip the lock release. It could before: a rejecting close() left the lock
      // file on disk, and the dictionary stayed unwritable until breakStaleLock aged it
      // out staleLockAgeMs later. Releasing the lock is the obligation that matters, so
      // it goes in the finally rather than the try.
      try {
        await this.closeLockHandle(handle);
      } finally {
